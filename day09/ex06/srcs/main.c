...
{
	i2c_rstart(EXP_ADDRESS, TW_WRITE);//SLA+W
	i2c_write(3);//tell expander I want to write to output1
	i2c_write(0);//clear all segments
	i2c_write(255 & ~(1 << seg));//tell expander which seg I want to update
	i2c_write(num);//display number
}
...
void	read_adc(uint8_t pin)
{
	...
	ADMUX &= ~(1 << ADLAR);//right adjust the result (push it to the left)	
...
int main()
{
	...
	uint8_t pins[10] = {LED0, LED1, LED2, LED3, LED4, LED5, LED6, LED7, LED8, LED9};
	uint16_t rv1 = 0;
	...
	while (1)
	{
		read_adc(0);//read rv1
		rv1 = ADC;
		set_led_seg(pins[rv1 % 10], DP4);
		set_led_seg(pins[(rv1 / 10) % 10], DP3);
		set_led_seg(pins[(rv1 / 100) % 10], DP2);
		set_led_seg(pins[rv1 / 1000], DP1);
	}
	i2c_stop();
	return 0;
}
