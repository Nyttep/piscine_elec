...
int main()
{
	...
	i2c_write(0b11110001);//set D9/D10/D11 as output;
	// i2c_rstart(EXP_ADDRESS, TW_WRITE);//SLA+W
	// i2c_write(2);//tell expander I want to write to output0
	// sei();
	while (1)
	{
		if (button_state1 == 0)
		{
			if (read_expander0(SW3) == 1) //checking if SW3 is set to 1 (button is pressed)
			{
				button_state1 = 1;
				if (counter < 7)
					counter++;
		...
		if (read_expander0(SW3) == 0) //checking if SW3 is set to 0 (button is not pressed)
			button_state1 = 0;
